...
I was inspired by this [post](https://stackoverflow.com/questions/47446458/ggextra-plot-format-similar-marginal-plots-for-different-plot-dimensions) and
this [post](http://faculty.missouri.edu/huangf/data/quantf/ancova_in_r_handout.pdf). 
...
res_omvar <- leveneTest(Posttest ~ Groups, center = mean, data = df_wide)
pvalue_omvar <- res_omvar$`Pr(>F)`[1]

# Results ANCOVA
res_anc <- aov(Posttest ~ Pretest + Groups, data = df_wide)
pvalue_anc <- summary(res_anc)[[1]][["Pr(>F)"]][2]
